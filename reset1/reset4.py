...
class Component:
    ...
    def __init__(self, label, lokatie, **kwargs):
        # These values will now influence dynamic spacing
        self.pixels_tussen_kringen_H = 0  # Horizontal spacing requirement for this component's "kringen" (vertical flow)
        self.pixels_tussen_takken_V = 0  # Vertical spacing requirement for this component's "takken" (horizontal flow)
        self.inputstub = 20
        self.outputstub = 20
        self.label = label
    ...
    def generate_svg(self):
        ...
        # --- CONFIGURATION ---
        MIN_NODE_SPACING = 30  # Minimum padding between component boundary and calculated spacing
        CANVAS_PADDING = 70  # Padding for the entire canvas

        # --- Find canvas dimensions and max component sizes and required spacing ---
        max_x, max_y = 0, 0
        max_component_width = 0
        max_component_height = 0

        # Track maximum required horizontal and vertical spacing from component properties
        max_required_h_spacing = 0
        max_required_v_spacing = 0
        ...
        while nodes_to_visit:
            ...
            # Update max component dimensions
            ...
            # Update max required spacing from component properties
            # This considers the component's own size plus its specified spacing requirements
            current_h_spacing_needed = node.boundarybox_breedte + node.pixels_tussen_kringen_H
            if current_h_spacing_needed > max_required_h_spacing:
                max_required_h_spacing = current_h_spacing_needed

            current_v_spacing_needed = node.boundarybox_hoogte + node.pixels_tussen_takken_V
            if current_v_spacing_needed > max_required_v_spacing:
                max_required_v_spacing = current_v_spacing_needed

            nodes_to_visit.extend(node.children)

        # Calculate dynamic spacing based on max component dimensions AND their internal spacing requirements
        # Ensure a minimum padding even if component-specific spacing is zero
        H_SPACING = max(max_required_h_spacing, max_component_width + MIN_NODE_SPACING)
        V_SPACING = max(max_required_v_spacing, max_component_height + MIN_NODE_SPACING)

        # Canvas size is based on grid plus the maximum component size for padding
        canvas_width = (max_x * H_SPACING) + max_component_width + CANVAS_PADDING * 2
        canvas_height = (max_y * V_SPACING) + max_component_height + CANVAS_PADDING * 2
        ...
        def get_center_coords(node):
            ...
            cx = (node.x * H_SPACING) + CANVAS_PADDING + (max_component_width / 2)
            cy = (node.y * V_SPACING) + CANVAS_PADDING + (max_component_height / 2)
            return cx, cy
...
if __name__ == "__main__":
    ...
    drawing.save_svg("reset4.svg")

    print("✅ SVG file 'reset4.svg' has been created successfully using drawsvg.")
