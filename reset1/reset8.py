...
class Component:
    ...
    def __init__(self, label, lokatie, **kwargs):
        ...
        self.kwargs = kwargs
        self.boundarybox_x = 30
        self.boundarybox_y = 50
    ...
    def compute_column_widths(self):
        ...
        col_widths = {}
        for node in nodes:
            width = 60 + getattr(node, 'boundarybox_x', 0)
...
if __name__ == "__main__":
    ...
    for node in all_nodes:
        ...
        cell.append(dw.Rectangle(x_pos, y_pos, rect_width , CELL_HEIGHT ,
                                 fill='lightblue', stroke='black', rx=5, ry=5))
        cell.append(dw.Text(node.label, 15, x_pos + 10, y_pos + 25,
                            font_family="Arial", fill="black"))
        cell.append(dw.Text(f"theory (x,y): ({node.x}, {node.y})", 12, x_pos + 10, y_pos + 50,
                            font_family="monospace", fill="#444"))
        cell.append(dw.Text(f"Grid (x,y): ({x_pos}, {y_pos})", 12, x_pos + 10, y_pos + 75,
                            font_family="monospace", fill="#444"))
        d.append(cell)
