...
class Component:
    ...
    def draw_recursive_top_left(self, canvas, x_spacing=50, y_spacing=50):
        # Convert grid coordinates (x, y) to Tkinter pixel positions
        pixel_x = (self.x * 1) #+ x_spacing
        pixel_y = (self.y * 1) # + y_spacing
        size = self.COMPONENT_SIZE
        # Draw square (no need to flip y)
        canvas.create_rectangle(pixel_x, pixel_y, pixel_x + size, pixel_y + size, fill="pink", outline="black")

        input_point = self.connectionpoint_input
        input_line_start = self.inputlinepoint
        if input_point and input_line_start:
            canvas.create_line(input_point[0], input_point[1],
                               input_line_start[0], input_line_start[1],
                               fill="red")
        ...
        for child in self.children:
            child_pixel_x = (child.x  * 1 )# * x_spacing
            child_pixel_y = (child.y * 1) # * y_spacing
            canvas.create_line(pixel_x + size/2, pixel_y + size/2,
                               child_pixel_x + size/2, child_pixel_y + size/2,
                               fill="black", width=2)
    ...
    def explode_coordinates_to_canvas(self, x=20, y=20):
    ...
    def inputlinepoint(self):
        ...
        inputline_lengte = 10 #hoelang moet het lijntje zijn van ons icoon
        if ip is None:
            return None
        if self.stroomrichting == "horizontal":
