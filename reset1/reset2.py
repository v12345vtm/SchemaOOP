...
class Component:
    ...
    def generate_svg(self):
        """Generates an SVG representation of the component tree."""
        # --- CONFIGURATION ---
        H_SPACING = 120  # Increased spacing for wider boxes
        V_SPACING = 100
        PADDING = 70
        ...
        # Canvas size is based on grid plus the size of the root component for padding
        canvas_width = (max_x * H_SPACING) + self.boundarybox_breedte + PADDING * 2
        canvas_height = (max_y * V_SPACING) + self.boundarybox_hoogte + PADDING * 2
        ...
        def get_center_coords(node):
            ...
            cx = (node.x * H_SPACING) + PADDING + (node.boundarybox_breedte / 2)
            cy = (node.y * V_SPACING) + PADDING + (node.boundarybox_hoogte / 2)
            return cx, cy
        ...
        for node in all_nodes:
            ...
            label_svg = f'<text x="{cx}" y="{cy}" class="component-label">{label_text}</text>'

            svg_elements.append(
                f'<rect x="{box_x}" y="{box_y}" width="{node_width}" height="{node_height}" class="component-box" rx="5"></rect>')
            svg_elements.append(label_svg)
            ...
            if node.parent:
                ...
                if node.parent.stroomrichting == 'vertical':
                    ...
                    connections.append(
                        f'<path class="connector-line" d="M{pcx},{pcy} L{pcx},{cy} M{pcx},{cy} L{cx},{cy}" />')
                else:  # horizontal
                    # Horizontal trunk, vertical branch
                    connections.append(f'<path class="connector-line" d="M{pcx},{pcy} L{cx},{pcy} L{cx},{cy}" />')

        # --- Assemble Final SVG ---
        svg_start = f'<svg width="{int(canvas_width)}" height="{int(canvas_height)}" xmlns="http://www.w3.org/2000/svg">\n'
        svg_style = """<style>
    .component-box { stroke: black; stroke-width: 2; fill: #f8f9fa; }
    .component-label { font-family: Arial, sans-serif; font-size: 10px; text-anchor: middle; alignment-baseline: middle; }
    .connector-line { stroke: #343a40; stroke-width: 1.5; fill: none; }
</style>\n"""
        svg_end = "</svg>"

        return svg_start + svg_style + "\n".join(connections) + "\n" + "\n".join(svg_elements) + "\n" + svg_end
...
if __name__ == "__main__":
    ...
    svg_output = te_tekenen_startpunt.generate_svg()

    # Save the SVG content to a file
    with open("out.svg", "w", encoding="utf-8") as f:
        f.write(svg_output)

    print("✅ SVG file 'schema.svg' has been created successfully.")