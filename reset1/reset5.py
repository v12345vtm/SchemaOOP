...
# --- All your component classes are included here (No changes needed in classes) ---
...
class Component:
    ...
    def __init__(self, label, lokatie, **kwargs):
        ...
        self.kwargs = kwargs
        # Default horizontal spacing for components
...
class Appliance(Component):
    ...
    def __init__(self, label, lokatie, **kwargs):
        ...
        # Custom horizontal spacing for this type
        self.tussenruimte_H = 50
...
if __name__ == "__main__":
    ...
    # 3. --- SVG Drawing Logic with Dynamic Column Widths ---
    print("\n--- Generating SVG file: tree_grid_dynamic.svg ---")

    # Drawing constants
    CELL_HEIGHT = 80  # Height can remain fixed or be made dynamic similarly
    H_PADDING = 10
    V_PADDING = 10

    # Get all nodes to determine layout
    all_nodes = te_tekenen_startpunt.get_all_nodes()
    if not all_nodes:
        print("Tree is empty, no SVG generated.")
        exit()

    max_x = max(node.x for node in all_nodes)
    max_y = max(node.y for node in all_nodes)

    # Step A: Group nodes by their column (x-coordinate)
    nodes_by_column = defaultdict(list)
    for node in all_nodes:
        nodes_by_column[node.x].append(node)

    # Step B: Calculate the width for each column based on its content
    column_widths = {}
    for x in range(max_x + 1):
        if x in nodes_by_column:
            # Find the max tussenruimte_H for all nodes in this column
            max_tussenruimte = max(node.tussenruimte_H for node in nodes_by_column[x])

            column_widths[x] = 60 + max_tussenruimte
        else:
            # Fallback for empty columns, though unlikely with this algorithm
            column_widths[x] = 60 + 50

    # Step C: Calculate the starting X position for each column
    column_x_positions = {}
    current_x = H_PADDING
    for x in range(max_x + 1):
        column_x_positions[x] = current_x
        current_x += column_widths[x]

    # Calculate final canvas dimensions
    canvas_width = current_x  # Total width is the final X position
    canvas_height = (max_y + 1) * CELL_HEIGHT + V_PADDING

    # Create the drawing canvas
    d = dw.Drawing(canvas_width, canvas_height, origin=(0, 0))
    d.append(dw.Rectangle(0, 0, canvas_width, canvas_height, fill='#f5f5f5'))

    # Step D: Iterate through all nodes and draw them using the dynamic values
    for node in all_nodes:
        # Get pre-calculated position and width for the node's column
        x_pos = column_x_positions[node.x]
        cell_width = column_widths[node.x]

        # Y position remains uniformly spaced
        y_pos = node.y * CELL_HEIGHT + V_PADDING

        cell = dw.Group(id=node.label)

        # Draw the cell rectangle with the dynamic width
        cell.append(dw.Rectangle(x_pos, y_pos, cell_width - H_PADDING, CELL_HEIGHT - V_PADDING,
    ...
    # Save the SVG file
    d.save_svg("tree_grid_dynamic.svg")
