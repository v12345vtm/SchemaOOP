...
class Component:
    ...
    def generate_svg(self):
        ...
        canvas_width = (max_x * H_SPACING) + BOX_WIDTH + PADDING * 2
        canvas_height = (max_y * V_SPACING) + BOX_HEIGHT + PADDING * 2
        ...
        def get_coords(node):
            # Center X, Center Y
            cx = (node.x * H_SPACING) + (BOX_WIDTH / 2) + PADDING
            cy = (node.y * V_SPACING) + (BOX_HEIGHT / 2) + PADDING
            return cx, cy
        ...
        while q:
            ...
            svg_elements.append(
                f'<rect x="{box_x}" y="{box_y}" width="{BOX_WIDTH}" height="{BOX_HEIGHT}" class="component-box" rx="5"></rect>')
            svg_elements.append(label_svg)

            # --- Draw Connection to Parent ---
            if node.parent:
                pcx, pcy = get_coords(node.parent)
...
if __name__ == "__main__":
    ...
    with open("reset1.svg", "w", encoding="utf-8") as f:
        f.write(svg_output)

    print("✅ SVG file 'reset1.svg' has been created successfully.")