...
class Component:
    ...
    def draw_stubs(self, drawing, x, y, w, h):
        """
        Calculates and draws both input and output stubs for the component.
        Returns:
            (effectieve_IN, effectieve_OUT)
        """
        # Calculate connection points
        conn_in, conn_out = self.get_startingpoints_stubs_from_rect(x, y, w, h)

        # Draw circles at the connection points
        self.svg_draw_circle(drawing, conn_in[0], conn_in[1], 6, fill="brown")
        self.svg_draw_circle(drawing, conn_out[0], conn_out[1], 6, fill="brown")

        # Draw the input and output stubs
        effectieve_IN = self.draw_input_stub(drawing, conn_in)
        effectieve_OUT = self.draw_output_stub(drawing, conn_out)

        # Draw circles at the effective ends of the stubs
        self.svg_draw_circle(drawing, effectieve_IN[0], effectieve_IN[1], 6, fill="red")
        self.svg_draw_circle(drawing, effectieve_OUT[0], effectieve_OUT[1], 6, fill="green")

        return effectieve_IN, effectieve_OUT
    ...
    # Get input and output connection points
        effectieve_IN, effectieve_OUT = self.draw_stubs(drawing, x, y, box_w, box_h)

        IN = self.svg_draw_text(drawing, "IN", effectieve_IN[0], effectieve_IN[1])
    ...
    def collect_properties_single_pass(self):
        """
        Traverses the tree once, populating all desired properties in a single pass:
        - Updates maxx, maxy for each node (including bounding from children)
        - Sorts children in place if needed
        - Caches other subtree properties if desired
        """

        def postorder(node):
            # Recursively process children first
            maxx = getattr(node, 'maxx', 0)
            maxy = getattr(node, 'maxy', 0)
            if hasattr(node, 'children'):
                # Optional: sort children here (e.g., by label)
                node.children.sort(key=lambda c: (Component.extract_int(c.label), c.label))
                for child in node.children:
                    postorder(child)
                    # Update maxx/maxy using children
                    maxx = max(maxx, getattr(child, 'maxx', 0))
                    maxy = max(maxy, getattr(child, 'maxy', 0))
            node.maxx = maxx
            node.maxy = maxy
            # Set more properties as needed, e.g., deepest_descendant_x/y if desired
            # node.deepest_x = max([node.x] + [getattr(ch, 'deepest_x', 0) for ch in node.children])
            # node.deepest_y = max([node.y] + [getattr(ch, 'deepest_y', 0) for ch in node.children])
            return node
    ...
    def explode_coordinates_to_canvas_pro(self, fixed_col_width=100, col_spacing=30):
        """
        Assigns grid_x for all nodes in the tree.
        - For most nodes, grid_x = node.x * (fixed_col_width + col_spacing)
        - For vertical nodes with a previous sibling (x-1) that's horizontal, grid_x = that sibling's grid_x + maxx + col_spacing
        (This prevents overlap when a wide horizontal item precedes a vertical one.)
        """
        # Gather all nodes and build a quick lookup by (x, y)
        all_nodes = list(self.walk_tree())
        grid_by_xy = {(n.x, n.y): n for n in all_nodes}

        # If not provided, pick a reasonable default column width
        if fixed_col_width is None:
            fixed_col_width = max((getattr(n, "boundarybox_breedte", 0) for n in all_nodes), default=50)

        for node in all_nodes:
            # Default: fixed column grid
            node.grid_x = node.x * (fixed_col_width + col_spacing)

            # Special: vertical node with horizontal predecessor left of it
            if node.stroomrichting == "vertical" and node.x > 0:
                prev = grid_by_xy.get((node.x - 1, node.y))
                if prev and getattr(prev, "stroomrichting", None) == "horizontal":
                    node.grid_x = prev.grid_x + getattr(prev, "maxx", fixed_col_width) + col_spacing
            # You may also want to adjust grid_y similarly, or leave it to row logic

        # (Optional!) For completeness, you can compute grid_y with a fixed row height, or customize similarly:
        fixed_row_height = max((getattr(n, "boundarybox_hoogte", 0) for n in all_nodes), default=50)
        row_spacing = 30
        for node in all_nodes:
            node.grid_y = node.y * (fixed_row_height + row_spacing)
...
if __name__ == "__main__":
    ...
    # Now batch-collect maxx, maxy, sorting, etc. in a single traversal:
    te_tekenen_startpunt.collect_properties_single_pass()

    # With updated sizes, calculate actual canvas positions
    te_tekenen_startpunt.explode_coordinates_to_canvas_pro()

    # Determine canvas size considering all components
    canvas_width, canvas_height = te_tekenen_startpunt.get_canvas_size()
    drawing = draw.Drawing(canvas_width + 100, canvas_height + 100)

    # Draw SVG using calculated canvas grid_x/grid_y positions
    te_tekenen_startpunt.draw_svg(drawing, 0, 60, swapy=True)

    # Save the final SVG
    drawing.save_svg('41.svg')
