...
class Component:
    ...
    def draw(self, c, COMPONENT_SIZE=20, font="Helvetica", font_size=6):
        ...
        # Draw input/output lines in blue
        c.setStrokeColorRGB(0, 0, 1)  # blue

        # Get connection points
        input_pt = self.connectionpoint_input
        output_pt = self.connectionpoint_output

        if self.stroomrichting == "horizontal":
            if input_pt:
                # Line from 10px left of input to input
                c.line(input_pt[0] - 10, input_pt[1], input_pt[0], input_pt[1])
            if output_pt:
                # Line from output to 10px right of output
                c.line(output_pt[0], output_pt[1], output_pt[0] + 10, output_pt[1])

        # You can later add vertical support similarly
        elif self.stroomrichting == "vertical":
            if input_pt:
                c.line(input_pt[0], input_pt[1] - 10, input_pt[0], input_pt[1])
            if output_pt:
                c.line(output_pt[0], output_pt[1], output_pt[0], output_pt[1] + 10)
