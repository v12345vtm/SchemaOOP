...
class oudePrieze(Component):
    _icon_cache = {}  # Class-level cache for images

    def __init__(self, label, type,
                 hydro=False,
                 kinderveiligheid=True,
                 aarding=True,
                 aantal=1,
                 **kwargs):
        super().__init__(label, type, **kwargs)
        ...
        self.aantal = aantal
        self._tk_img = None  # Instance keeps a strong reference!

    def get_icon_path(self):
        # Use your image naming convention
        return (
            f"symbolen/prieze_hydro_{str(self.hydro).lower()}"
    ...
    def draw_recursive_top_left(self, canvas, x_spacing=0, y_spacing=0, swapy=False):
        ...
        canvas.create_rectangle(pixel_x, pixel_y, pixel_x + size, pixel_y + size, fill="blue", outline="black")

        # --- Draw the icon image (PNG) ---
        icon_path = self.get_icon_path()
        try:
            if icon_path not in Prieze._icon_cache:
                from PIL import Image
                img = Image.open(icon_path).resize((size, size), Image.Resampling.LANCZOS)
                Prieze._icon_cache[icon_path] = ImageTk.PhotoImage(img)
            self._tk_img = Prieze._icon_cache[icon_path]  # strong reference required!
            canvas.create_image(pixel_x, pixel_y, anchor='nw', image=self._tk_img)
        except Exception as e:
            print(f"Image error: {icon_path} {e}")

        # Continue with the standard rest (dots, stubs, connections, children)
        super().draw_recursive_top_left(canvas, x_spacing, y_spacing, swapy=False)
...
zek1001.add_child(vaatwas301 , prieze)
